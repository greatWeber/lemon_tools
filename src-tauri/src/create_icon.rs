 ...
 */
 pub async fn create(base64_string: &str,sizes: &Vec<String>,path: &str, format: &str) -> Result<(), Box<dyn Error>>{
  ...
  async fn save_icon(image: &image::DynamicImage, size: &str, path: &str,format_str: &str)-> Result<(),Box<dyn Error>> {
  ...
  let handles: Vec<_> = sizes.iter().map(|s| {
    let s = s.to_string();
