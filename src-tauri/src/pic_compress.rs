use std::{error::Error, fs::{self, File}, io::{Read, Write}};
...
pub async fn compress_pic(path: &str)-> Result<(),Box<dyn Error>>{
  ...
  let extension = new_path.extension().and_then(std::ffi::OsStr::to_str).unwrap();
  println!("The file extension is: {}", extension);
  if extension == "png" {
    let options = Options::from_preset(6);
    let input_path = InFile::Path(new_path.to_path_buf());
    let output_path = OutFile::Path{path:Some(new_path.to_path_buf()),preserve_attrs:true};
    ...
    tokio::task::spawn_blocking(move || {
      ...
      // 使用编码器将颜色数据写入到新的文件中
      encoder.encode(&raw_data, dimensions.0, dimensions.1, image::ColorType::Rgba8).unwrap();
    }).await.unwrap();
...
pub async fn compress_dir(path: &str)-> Result<(),Box<dyn Error>> {
  ...
  if dir.exists() && dir.is_dir() {
    ...
    for entry in entries {
      ...
      let path = entry.path();
      if path.is_file() && pic_type.contains(&path.extension().unwrap_or_default().to_str().unwrap()) {
    ...
    let handles: Vec<_> = path_array_clone.iter().map(|path| {
